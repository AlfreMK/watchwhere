...
function Movie() {
  ...
  const [country, setCountry] = useState(
    localStorage.getItem("country") || "CL"
  );
        ...
        <Container>
          ...
          <Info>
            <Dropdown>
              ...
              >
                <img
                  className="inline-flex w-6 h-4 mr-2 pointer-events-none"
                  src={findFlagUrlByIso2Code(country)}
                  alt={country}
                />
                {country}
              ...
              <div
                id="dropdown"
                className={
                  "top-12 absolute bg-indigo-900 divide-y divide-gray-100 rounded-lg shadow-xl cursor-pointer " +
              ...
              >
                <ul className="text-sm overflow-y-scroll h-40">
                  {countries.map((country) => (
                    <li
                      key={country}
                      className="py-2 px-6 hover:bg-indigo-800 rounded-lg"
                      onClick={() => {
                        setCountry(country);
                        setDropdown(false);
